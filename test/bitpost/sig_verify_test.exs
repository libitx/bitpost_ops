defmodule Bitpost.SigVerifyTest do
  ...
  describe "simple example without signed content" do
    ...
    test "works consistently if index is a binary integer", ctx do
      res = %Operate.Cell{op: ctx.op, data_index: 0, params: [<<1>>, "##dummy_sig1##", "1iCqLKPjv5HZ43MPkAC42vKPANLkGzbKF"]}
      |> Operate.Cell.exec!(ctx.vm)
      |> Map.get("signatures")
      |> List.first
    ...
    test "must raise when signature or pubkey are missing", ctx do
      assert_raise RuntimeError, ~r/^Lua Error/, fn ->
        %Operate.Cell{op: ctx.op, params: ["1", nil, "1iCqLKPjv5HZ43MPkAC42vKPANLkGzbKF"]}
        |> Operate.Cell.exec!(ctx.vm)
      end

      assert_raise RuntimeError, ~r/^Lua Error/, fn ->
        %Operate.Cell{op: ctx.op, params: ["1", "##dummy_sig1##", nil]}
        |> Operate.Cell.exec!(ctx.vm)
      end
    end
  end
  ...
  describe "verifying a signature" do
    ...
    test "must verify with raw signatures", ctx do
      sig = Base.decode64! "IF4c9E2d7d0eqkR7apt8ZGXpthYhb4eF6ifLp5gXbIsRTw5GzNmK2H7kMjP1nYez0l15R5fLz48HBfqMJEocHGE="
      res = %Operate.Cell{op: ctx.op, data_index: 0, params: ["1", sig, "1iCqLKPjv5HZ43MPkAC42vKPANLkGzbKF"]}
      |> Operate.Cell.exec!(ctx.vm)
      |> Map.get("signatures")
      |> List.first
    ...
    test "must verify a correct timestamped signature", ctx do
      sig = "IPOJjDEQC2s44zbZCEPpjjFUA6w8DmMbpqpijSn0k44xQco4AU0RBp2aBZ3H/KgV3+u0l3e6YFeVNOidT0z0nbY="
